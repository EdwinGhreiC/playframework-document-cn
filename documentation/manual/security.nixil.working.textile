...
bq. 这种攻击方法的前提是用户(在当前或者最近一段时间)登录了你的应用，浏览器中还保留着的cookie。此时如果在该用户访问的某个页面(可能是任何一个其他的网站)中引入一段恶意的代码或者链接，使之向你的应用发起请求。那由于这个用户的session还没有过期，该请求就会利用这个伪造的session来通过应用的认证，执行恶意的操作了。
